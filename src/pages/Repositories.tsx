...
const platformIcon = { github: <Github className="h-4 w-4" />, gitlab: <GitlabIcon className="h-4 w-4" />, bitbucket: <span className="text-xs font-bold">BB</span> };
          ...
          <div key={r.id} className="glass p-5 hover:bg-white/[0.08] transition-all">
            <div className="flex items-start justify-between mb-3">
              ...
              <div className="relative w-10 h-10 shrink-0">
                <svg viewBox="0 0 40 40" className="-rotate-90">
                  ...
                  <circle cx="20" cy="20" r="16" fill="none" stroke={r.score >= 80 ? "hsl(142 76% 36%)" : r.score >= 60 ? "hsl(39 100% 50%)" : "hsl(0 84% 60%)"} strokeWidth="3" strokeDasharray={`${r.score} 100`} strokeLinecap="round" />
                </svg>
            ...
            <div className="flex gap-2">
              <Button size="sm" className="flex-1 gradient-primary text-primary-foreground border-0 text-xs" asChild>
                <Link to={`/repositories/${r.id}`}>View Architecture</Link>
              </Button>
              <Button size="sm" variant="ghost" className="border border-white/10 text-foreground text-xs" onClick={() => toast({ title: "Scan started!", description: `Scanning ${r.org}/${r.name}...` })}>
                Scan Now
              </Button>
            </div>
          </div>
        ...
        <DialogContent className="glass border-white/10 bg-card sm:max-w-md">
          ...
          {step === 1 && (
            <div className="grid grid-cols-3 gap-3">
              {[{ name: "GitHub", icon: <Github className="h-6 w-6" /> }, { name: "GitLab", icon: <GitlabIcon className="h-6 w-6" /> }, { name: "Bitbucket", icon: <span className="text-lg font-bold">BB</span> }].map((p) => (
