...
import { Zap, LayoutDashboard, FolderGit2, ScanSearch, Lightbulb, DollarSign, Settings, FileText, Bell, Menu, X, ChevronLeft } from "lucide-react";
      ...
      <header className="h-14 border-b border-white/5 flex items-center justify-between px-4 lg:px-6 shrink-0 relative z-20 bg-background/80 backdrop-blur-md">
        <div className="flex items-center gap-4">
          ...
          <nav className="hidden md:flex items-center gap-6 ml-8 text-sm">
            {["Dashboard", "Repositories", "Insights", "Cost Analysis"].map((l) => (
              <Link key={l} to={l === "Dashboard" ? "/dashboard" : l === "Repositories" ? "/repositories" : l === "Insights" ? "/insights" : "/dashboard"} className="text-muted-foreground hover:text-foreground transition-colors">
                {l}
              </Link>
            ))}
          </nav>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className={cn(
          ...
          <nav className="flex-1 px-2 space-y-1">
            {navItems.map((item) => {
              const active = location.pathname === item.path || (item.path === "/repositories" && location.pathname.startsWith("/repositories"));
              return (
                <Link key={item.label} to={item.path} onClick={() => setMobileOpen(false)} className={cn(
