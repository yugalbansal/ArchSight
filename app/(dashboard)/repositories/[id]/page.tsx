...
const nodeTypeColors: Record<string, string> = {
    api: "bg-sky-400", service: "bg-indigo-400", database: "bg-green-400",
    cache: "bg-yellow-400", llm: "bg-purple-400", external: "bg-sky-400", worker: "bg-orange-400",
};

export default function RepositoryDetail({ params }: { params: Promise<{ id: string }> }) {
    // Next.js 15: params is a Promise, use React.use() to read it in client components
    const { id } = use(params);
            ...
            <Tabs defaultValue="overview">
                <TabsList className="bg-white/5 border border-white/10">
                    {["overview", "architecture", "insights", "cost", "history"].map((t) => (
                        <TabsTrigger key={t} value={t} className="data-[state=active]:bg-primary/20 data-[state=active]:text-primary capitalize text-muted-foreground">
                            {t === "cost" ? "Cost Analysis" : t}
                        </TabsTrigger>
                    ))}
                </TabsList>
                    ...
                    <div>
                        ...
                        <div className="grid md:grid-cols-3 gap-3">
                            {["Switch /api/chat to Claude Haiku — save ~$380/mo", "Add Redis caching for /api/summarize — reduce calls by 60%", "Enable batch embeddings — save ~$20/mo"].map((s, i) => (
                                <div key={i} className="glass p-4"><p className="text-sm text-foreground">{s}</p></div>
                            ))}
                        </div>
                    </div>
